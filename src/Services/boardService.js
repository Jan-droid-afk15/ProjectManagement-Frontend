...
import { fetchingStart, fetchingFinish } from "../Redux/Slices/userSlice";
...
const listRoute = "http://localhost:3030/list";
const boardRoute = "http://localhost:3030/board";
...
export const inviteUnregisteredUserToBoard = async (
  boardId,
  email,
  dispatch
) => {
  ...
  try {
    const res = await axios.post(`${boardRoute}/${boardId}/unregistered-user-to-board`, {
      email,
    });
    dispatch(
      openAlert({
